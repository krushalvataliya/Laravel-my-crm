@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.organizations')) }}
        @endslot

        @slot('actions')
            @include('my-crm::partials.filters', [
                'action' => route('laravel-crm.organisations.filter'),
                'model' => '\Kv\MyCrm\Models\Organisation'
            ])
            @can('create crm organisations')
            <span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.organisations.create')) }}"><span class="fa fa-plus"></span>  {{ ucfirst(__('my-crm::lang.add_organization')) }}</a></span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        ...
        <table class="table mb-0 card-table table-hover">
            <thead>
            <tr>
                <th scope="col" colspan="2">@sortablelink('name', ucwords(__('my-crm::lang.name')))</th>
                <th scope="col">@sortablelink('organisationType.name', ucwords(__('my-crm::lang.type')))</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.labels')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.open_deals')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.lost_deals')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.won_deals')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.next_activity')) }}</th>
            ...
            @foreach($organisations as $organisation)
                <tr class="has-link" data-url="{{ url(route('laravel-crm.organisations.show',$organisation)) }}">
                    ...
                    <td></td>
                    <td>{{ $organisation->ownerUser->name ?? ucfirst(__('my-crm::lang.unallocated')) }}</td>
