            ...
            <div class="col col-md-10">
                <div class="card">
                    <div class="card-header">
                        <h3 class="mb-0">{{ ucfirst(__('my-crm::lang.pipeline_stage')) }}: {{ $pipelineStage->name }} <span class="float-right">
                            <a type="button" class="btn btn-outline-secondary btn-sm" href="{{ url(route('laravel-crm.pipeline-stages.index')) }}"><span class="fa fa-angle-double-left"></span> {{ ucfirst(__('my-crm::lang.back_to_pipeline_stages')) }}</a> | 
                                ...
                                @can('delete crm pipelines')
                                    <form action="{{ route('laravel-crm.pipeline-stages.destroy',$pipelineStage) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                                        {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
                                        <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.pipeline') }}" {{ ($pipelineStage->system == 1) ? 'disabled' : null }}><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                                    </form>
                                @endcan
                        </span></h3>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <h6 class="text-uppercase section-h6-title"><span>{{ ucfirst(__('my-crm::lang.pipeline')) }}</span></h6>
                                <hr />
