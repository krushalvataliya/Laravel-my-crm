@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.purchase_orders')) }}
        @endslot

        @slot('actions')
            @include('my-crm::partials.filters', [
                'action' => route('laravel-crm.purchase-orders.filter'),
                'model' => '\Kv\MyCrm\Models\PurchaseOrder'
            ])
            @can('create crm purchase orders')
            <span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.purchase-orders.create')) }}"><span class="fa fa-plus"></span>  {{ ucfirst(__('my-crm::lang.add_purchase_order')) }}</a></span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        <table class="table mb-0 card-table table-hover">
            ...
            <tr>
                ...
                @endhasordersenabled
                <th scope="col">{{ ucwords(__('my-crm::lang.supplier')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.issue_date')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.delivery_date')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.delivery_type')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.amount')) }}</th>
                        ...
                        @if(! $purchaseOrder->xeroPurchaseOrder)
                            ...
                            @can('delete crm purchase orders')
                                <form action="{{ route('laravel-crm.purchase-orders.destroy',$purchaseOrder) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
                                    <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.purchase_order') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                                </form>
                            @endcan
                        @endif
                        @if($purchaseOrder->xeroPurchaseOrder)
