<div class="form-group @error($name) text-danger @enderror">
    ...
    <textarea class="form-control @error($name) is-invalid @enderror" id="textarea_{{ $name }}" name="{{ $name }}" rows="{{ $rows ?? 3 }}" @include('my-crm::partials.form.attributes')>{{ $value ?? null }}</textarea>
    @error($name)
