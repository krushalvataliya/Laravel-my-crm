<div class="form-group @error($name) text-danger @enderror">
    ...
    @isset($prepend)
        <div class="input-group">
            ...
            @endisset
            <input id="input_{{ $name }}" type="password" name="{{ $name }}" value="{{ $value ?? null }}" class="form-control @error($name) is-invalid @enderror" @include('my-crm::partials.form.attributes') >
            @isset($prepend)
        </div>
    @endisset
    @error($name)
