    ...
    @component('my-crm::components.card-header')
        ...
        @slot('actions')
            <span class="float-right">
                <a type="button" class="btn btn-outline-secondary btn-sm" href="{{ url(route('laravel-crm.teams.index')) }}"><span class="fa fa-angle-double-left"></span> {{ ucfirst(__('my-crm::lang.back_to_teams')) }}</a> | 
                ...
                <form action="{{ route('laravel-crm.teams.destroy',$team) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                    {{ method_field('DELETE') }}
                    {{ csrf_field() }}
                    <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.team') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                </form>
            </span>
        @endslot
        ...
        <div class="row">
            <div class="col-sm-6 border-right">
                ...
                <dl class="row">
                    ...
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.created_by')) }}</dt>
                    ...
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.updated')) }}</dt>
                    <dd class="col-sm-9">{{ $team->updated_at->format($dateFormat)}}</dd>
                </dl>
                <h6 class="text-uppercase mt-4 section-h6-title"><span>{{ ucfirst(__('my-crm::lang.users')) }} ({{ $team->users->count() }})</span></h6>
                <hr />
            ...
            <div class="col-sm-6">
                <h6 class="text-uppercase">{{ ucfirst(__('my-crm::lang.activities')) }}</h6>
                <hr />
                ...
            </div>
        </div>
