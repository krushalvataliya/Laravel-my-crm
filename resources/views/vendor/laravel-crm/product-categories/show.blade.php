            ...
            <div class="col col-md-10">
                <div class="card">
                    <div class="card-header">
                        <h3 class="mb-0">{{ ucfirst(__('my-crm::lang.product_category')) }}: {{ $productCategory->name }} <span class="float-right">
                <a type="button" class="btn btn-outline-secondary btn-sm" href="{{ url(route('laravel-crm.product-categories.index')) }}"><span class="fa fa-angle-double-left"></span> {{ ucfirst(__('my-crm::lang.back_to_product_categories')) }}</a> | 
                @can('edit crm product categories')
                    ...
                    {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
                    <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.product_category') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                </form>
                                @endcan
            </span></h3>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                @can('view crm products')
                                    <h6 class="text-uppercase section-h6-title-table"><span>{{ ucfirst(__('my-crm::lang.products')) }} ({{ $productCategory->products->count() }})</span></h6>
                                    <table class="table table-hover">
                                        <thead>
                                        <tr>
                                            <th scope="col">{{ ucfirst(__('my-crm::lang.item')) }}</th>
                                            <th scope="col" width="120">{{ ucfirst(__('my-crm::lang.price')) }} ({{ \Kv\MyCrm\Models\Setting::currency()->value ?? 'USD' }})</th>
                                            <th scope="col" width="80">{{ ucfirst(__('my-crm::lang.tax')) }} %</th>
                                            <th scope="col" width="80">{{ ucfirst(__('my-crm::lang.active')) }}</th>
                                        </tr>
                                        </thead>
                                        <tbody>
