@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ $invoice->title }}
            @if($invoice->sent == 1)
                <small><span class="badge badge-success">{{ ucfirst(__('my-crm::lang.sent')) }}</span></small>
            @endif
        ...
        @slot('actions')
            <span class="float-right">
                ...
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('laravel-crm.invoices.download', $invoice) }}">{{ ucfirst(__('my-crm::lang.download')) }}</a>
                @if(! $invoice->xeroInvoice)
                    ...
                    @endif
                @endif
                @include('my-crm::partials.navs.activities') @if($invoice->amount_paid <= 0) |
                @if(! $invoice->xeroInvoice)
                    ...
                    <form action="{{ route('laravel-crm.invoices.destroy', $invoice) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
                        <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.invoice') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                    </form>
                    @endcan
                    @endif  
                @endif
    ...
    @component('my-crm::components.card-body')
        ...
        <div class="row card-show card-fa-w30">
            <div class="col-sm-6 border-right">
                ...
                <h6 class="text-uppercase mt-4 section-h6-title-table"><span>{{ ucfirst(__('my-crm::lang.invoice_lines')) }} ({{ $invoice->invoiceLines->count() }})</span></h6>
