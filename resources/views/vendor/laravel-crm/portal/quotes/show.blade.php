            ...
            <div class="container-fluid">
                <h1 class="navbar-brand mb-0" href="#">
                    ...
                    @if($quote->accepted_at)
                        <small><span class="badge badge-success">{{ ucfirst(__('my-crm::lang.accepted')) }}</span></small>
                    @elseif($quote->rejected_at)
                        <small><span class="badge badge-danger">{{ ucfirst(__('my-crm::lang.rejected')) }}</span></small>
                    @elseif(\Carbon\Carbon::now() <= $quote->expire_at)
                        <small><span class="badge badge-secondary">{{ ucfirst(__('my-crm::lang.expires_in')) }} {{ $quote->expire_at->diffForHumans() }}</span></small>
                    @else
                        <small><span class="badge badge-danger">{{ ucfirst(__('my-crm::lang.quote_expired')) }}</span></small>
                    @endif
                </h1>
                    ...
                    <ul class="navbar-nav ml-auto">
                        @if($quote->accepted_at)
                            <div class="alert alert-success m-0" role="alert">
                                {{ ucfirst(__('my-crm::lang.you_have_accepted_this_quote')) }}.
                            </div>
                        @elseif($quote->rejected_at)
                            <div class="alert alert-danger m-0" role="alert">
                                {{ ucfirst(__('my-crm::lang.you_have_rejected_this_quote')) }}.
                            </div>
                        @elseif(\Carbon\Carbon::now() <= $quote->expire_at)
                            <li class="nav-item mr-2">
                                <form action="{{ url()->current() }}?signature={{ request()->input('signature') }}&expires={{ request()->input('expires') }}" method="POST" class="form-check-inline mr-0">
                                    ...
                                    <button class="btn btn-outline-success" type="submit">{{ ucfirst(__('my-crm::lang.accept')) }}</button>
                                </form>
                            </li>
                            <li class="nav-item mr-2">
                                <form action="{{ url()->current() }}?signature={{ request()->input('signature') }}&expires={{ request()->input('expires') }}" method="POST" class="form-check-inline mr-0">
                                    ...
                                    <button class="btn btn-outline-danger" type="submit">{{ ucfirst(__('my-crm::lang.reject')) }}</button>
                                </form>
                            </li>
                        @endif
                        <li class="nav-item">
                            <form action="{{ url()->current() }}?signature={{ request()->input('signature') }}&expires={{ request()->input('expires') }}" method="POST" class="form-check-inline mr-0">
                                ...
                                <button class="btn btn-outline-secondary" type="submit"><span class="fa fa-download" aria-hidden="true"></span> {{ ucfirst(__('my-crm::lang.download')) }}</button>
                            </form>
                        </li>
                    </ul>
                </div>
                ...
                <div class="card-body p-0">
                    <div class="row">
                        <div class="col px-5 py-4">
                            <h1 class="card-title pricing-card-title py-3 m-0 text-uppercase">{{ ucfirst(__('my-crm::lang.quote')) }}</h1>
                            @if($quote->reference)
                                <p class="mb-0"><strong>{{ ucfirst(__('my-crm::lang.reference')) }}</strong> {{ $quote->reference }}</p>
                            @endif
                            @if($quote->issue_at)
                                <p class="mb-0"><strong>{{ ucfirst(__('my-crm::lang.issue_date')) }}</strong> {{ $quote->issue_at->format($dateFormat) }}</p>
                            @endif
                            @if($quote->expire_at)
                                <p class="mb-0"><strong>{{ ucfirst(__('my-crm::lang.expiry_date')) }}</strong>  {{ $quote->expire_at->format($dateFormat) }}</p>
                            @endif
                        </div>
                    ...
                    <div class="row">
                        <div class="col px-5 py-4">
                            <div class="row py-1">
                                <div class="col-3">
                                    <strong>{{ ucfirst(__('my-crm::lang.issued_to')) }}</strong>
                                </div>
                                <div class="col">
                        ...
                        <div class="col px-5 py-4">
                            <div class="row py-1">
                                <div class="col-3">
                                    <strong>{{ ucfirst(__('my-crm::lang.from')) }}</strong>
                                </div>
                                <div class="col">
                                    {{ $fromName }}<br />
                    ...
                    <div class="row py-1">
                        <div class="col px-5 py-4">
                            <h5>{{ ucfirst(__('my-crm::lang.description')) }}</h5>
                        ...
                        <div class="col px-5 py-1">
                            <table class="table table-hover mb-0">
                                ...
                                <tr>
                                    ...
                                    <th scope="col">{{ ucfirst(__('my-crm::lang.comments')) }}</th>
                                </tr>
                                </thead>
                                <tbody>
                    ...
                    <div class="row py-1">
                        <div class="col px-5 py-4">
                            <h5>{{ ucfirst(__('my-crm::lang.terms')) }}</h5>
                            {!! nl2br($quote->terms) !!}
                        </div>
                    </div>
