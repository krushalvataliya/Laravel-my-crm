<div class="files">
    {{--<h6 class="text-uppercase">{{ ucfirst(__('my-crm::lang.files')) }}</h6>
    <hr />--}}
    @if($showForm)
        <form wire:submit.prevent="upload" id="inputUploadForm">
            @include('my-crm::livewire.components.partials.file.form-fields')
            <div class="form-group">
                <button type="submit" class="btn btn-primary">{{ ucfirst(__('my-crm::lang.upload')) }}</button>
            </div>
        </form>
    <hr/>
    @endif
