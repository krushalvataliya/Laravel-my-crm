<tr>
    ...
    <td>
        @include('my-crm::livewire.components.partials.task.status', ['task' => $task])
    </td>
    <td>{{ $task->name }}<br />
        @include('my-crm::tasks.partials.task-model', ['task' => $task])
    </td>
    ...
    <td class="disable-link text-right">
        @can('edit crm tasks')
            @if(! $task->completed_at)
                <a href="{{  route('laravel-crm.tasks.complete',$task) }}" class="btn btn-success btn-sm">{{ ucfirst(__('my-crm::lang.complete')) }}</a>
            @endif
        ...
        <form action="{{ route('laravel-crm.tasks.destroy',$task) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
            {{ method_field('DELETE') }}
            {{ csrf_field() }}
            <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.task') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
        </form>
        @endcan
    </td>
</tr>
