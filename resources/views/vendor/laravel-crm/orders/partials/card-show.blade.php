    ...
    @component('my-crm::components.card-header')
        ...
        @slot('actions')
            <span class="float-right">
                ...
                @can('edit crm orders')
                    ...
                    @else
                        @if(! $order->deliveryComplete())
                            ...
                            <a href="{{ route('laravel-crm.purchase-orders.create',['model' => 'order', 'id' => $order->id]) }}" class="btn btn-success btn-sm">{{ ucwords(__('my-crm::lang.purchase_order')) }}</a>
                            @endhaspurchaseordersenabled
                        @endif
                        @if(! $order->invoiceComplete())
                            @hasinvoicesenabled
                            <a href="{{ route('laravel-crm.invoices.create',['model' => 'order', 'id' => $order->id]) }}" class="btn btn-success btn-sm">{{ ucwords(__('my-crm::lang.generate_invoice')) }}</a>
                            @endhasinvoicesenabled
                        @endif
                        @if(! $order->deliveryComplete())
                            @hasdeliveriesenabled
                                <a href="{{ route('laravel-crm.deliveries.create',['model' => 'order', 'id' => $order->id]) }}" class="btn btn-success btn-sm">{{ ucwords(__('my-crm::lang.create_delivery')) }}</a>
                ...
                @can('view crm orders')
                    @if(! $subTotalError && ! $taxError && ! $totalError)
                        <a class="btn btn-outline-secondary btn-sm" href="{{ route('laravel-crm.orders.download', $order) }}">{{ ucfirst(__('my-crm::lang.download')) }}</a>
                    @endif
                @endcan    
                ...
                <form action="{{ route('laravel-crm.orders.destroy', $order) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                    {{ method_field('DELETE') }}
                    {{ csrf_field() }}
                    <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.order') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                </form>
                @endcan
            </span>
        @endslot
        ...
        <div class="row card-show card-fa-w30">
            <div class="col-sm-6 border-right">
                ...
                <dl class="row">
                    ...
                    @hasquotesenabled
                    @if($order->quote)
                        <dt class="col-sm-4 text-right">{{ ucfirst(__('my-crm::lang.quote')) }}</dt>
                    ...
                    <dd class="col-sm-8">@include('my-crm::partials.labels',[
                            'labels' => $order->labels
                    ])</dd>
                    <dt class="col-sm-4 text-right">Owner</dt>
                    <dd class="col-sm-8">@if($order->ownerUser)<a href="{{ route('laravel-crm.users.show', $order->ownerUser) }}">{{ $order->ownerUser->name ?? null }}</a> @else  {{ ucfirst(__('my-crm::lang.unallocated')) }} @endif</dd>
                </dl>
                ...
                <h6 class="text-uppercase mt-4 section-h6-title-table"><span>{{ ucfirst(__('my-crm::lang.order_items')) }} ({{ $order->orderProducts->count() }})</span></h6>
                <table class="table table-hover">
                    ...
                    @foreach($order->orderProducts()->whereNotNull('product_id')->get() as $orderProduct)
                        <tr>
                            ...
                            <td>
                                @if(! \Kv\MyCrm\Http\Helpers\CheckAmount\lineAmount($orderProduct))
                    ...
                    <tr>
                        ...
                        <td>
                            @if(! \Kv\MyCrm\Http\Helpers\CheckAmount\subTotal($order))
                    ...
                    <tr>
                        ...
                        <td>
                            @if(! \Kv\MyCrm\Http\Helpers\CheckAmount\tax($order))
                                <span data-toggle="tooltip" data-placement="top" title="Error with sub total" class="text-danger">
