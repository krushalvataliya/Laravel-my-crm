@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.invoices')) }}
        @endslot

        @slot('actions')
            @include('my-crm::partials.filters', [
                'action' => route('laravel-crm.invoices.filter'),
                'model' => '\Kv\MyCrm\Models\Invoice'
            ])
            @can('create crm invoices')
            <span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.invoices.create')) }}"><span class="fa fa-plus"></span>  {{ ucfirst(__('my-crm::lang.add_invoice')) }}</a></span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        <table class="table mb-0 card-table table-hover">
            ...
            <tr>
                ...
                @endhasordersenabled
                <th scope="col">{{ ucwords(__('my-crm::lang.to')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.date')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.due_date')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.overdue_by')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.paid_date')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.paid')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.due')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.sent')) }}</th>
