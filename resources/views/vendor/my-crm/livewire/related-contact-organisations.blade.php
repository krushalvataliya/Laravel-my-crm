<div>
    @can('view crm contacts')
        <h6 class="text-uppercase mt-4 section-h6-title"><span>{{ ucfirst(__('my-crm::lang.organizations')) }} ({{ $contacts->count() }})</span>@can('create crm contacts') @if($actions)<span class="float-right"><a href="#" data-toggle="modal" data-target="#linkOrganisationModal" class="btn btn-outline-secondary btn-sm"><span class="fa fa-plus" aria-hidden="true"></span></a></span>@endif @endcan</h6>
        <hr />
        @foreach($contacts as $contact)
            <p><span class="fa fa-building mr-1" aria-hidden="true"></span> <a href="{{ route('laravel-crm.organisations.show',$contact->entityable) }}">{{ $contact->entityable->name }}</a> @if($actions)<span class="float-right"><button wire:click.prevent="remove({{ $contact->entityable->id }})" type="button" class="btn btn-outline-danger btn-sm"><span class="fa fa-remove"></span></button></span>@endif</p>
        @endforeach
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="linkOrganisationModalLabel">{{ ucfirst(__('my-crm::lang.link_an_organization')) }} </h5>
                    ...
                    <form wire:submit.prevent="link">
                        <div class="modal-body autocomplete">
                            ...
                            <div class="form-group @error('organisation_name') text-danger @enderror">
                                <label>{{ ucfirst(__('my-crm::lang.organization_name')) }}</label>
                        ...
                        <div class="modal-footer">
                            ...
                            <button  wire:click.prevent="link()" type="button" class="btn btn-primary">{{ ucwords(__('my-crm::lang.link_organization')) }}</button>
                        </div>
                    </form>
                </div>
            </div>
