    ...
    <div class="card w-100 mb-2">
        <div class="card-body">
            <div class="media-body">
                <a href="#download" wire:click.prevent="download()">{{ $file->name }}</a>@include('my-crm::livewire.components.partials.file.actions', ['file' => $file])
                @if($showRelated)
                    <p class="pb-0 mb-2">
                        @if($file->filable instanceof \Kv\MyCrm\Models\Person)
                            <span class="fa fa-user-circle" aria-hidden="true"></span> <a
                                    href="{{ route('laravel-crm.people.show', $file->filable) }}">{{ $file->filable->name }}</a>
                        @elseif($file->filable instanceof \Kv\MyCrm\Models\Organisation)
