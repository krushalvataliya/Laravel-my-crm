            ...
            <div class="container-fluid">
                <h1 class="navbar-brand mb-0" href="#">
                    ...
                    @if($invoice->fully_paid_at)
                        <small><span class="badge badge-success">{{ ucfirst(__('my-crm::lang.paid')) }}</span></small>
                    @elseif(! $invoice->fully_paid_at && $invoice->due_date->diffinDays() > 0  && $invoice->due_date >= \Carbon\Carbon::now()->timezone($timezone))
                        <small><span class="badge badge-secondary">{{ ucfirst(__('my-crm::lang.due_in')) }} {{ $invoice->due_date->diffForHumans(false, true) }} </span></small>
                    @elseif(! $invoice->fully_paid_at && $invoice->due_date->diffinDays() <= 0  && $invoice->due_date >= \Carbon\Carbon::now()->timezone($timezone))
                        <small><span class="badge badge-secondary">{{ ucfirst(__('my-crm::lang.due_tomorrow')) }}</span></small>
                    @elseif(! $invoice->fully_paid_at && $invoice->due_date->diffinDays() > 0  && $invoice->due_date < \Carbon\Carbon::now()->timezone($timezone))
                        <small><span class="badge badge-danger">{{ $invoice->due_date->diffForHumans(false, true) }} {{ ucfirst(__('my-crm::lang.overdue')) }} </span></small>
                    @endif
                </h1>
                    ...
                    <ul class="navbar-nav ml-auto">
                       {{--@if($invoice->fully_paid_at)
                            <div class="alert alert-success m-0" role="alert">
                                {{ ucfirst(__('my-crm::lang.you_have_paid_this_invoice')) }}.
                            </div>
                        @else
                            <li class="nav-item mr-2">
                                <form action="{{ url()->current() }}?signature={{ request()->input('signature') }}" method="POST" class="form-check-inline mr-0">
                                    ...
                                    <button class="btn btn-outline-success" type="submit">{{ ucfirst(__('my-crm::lang.pay_now')) }}</button>
                                </form>
                            </li>
                        @endif--}}
                        <li class="nav-item">
                ...
                <div class="card-body p-0">
                    <div class="row">
                        <div class="col px-5 py-4">
                            <h1 class="card-title pricing-card-title py-4 m-0">{{ ucfirst(__('my-crm::lang.invoice')) }}</h1>
                        </div>
                    ...
                    <div class="row">
                        <div class="col px-5 py-4">
                            <div class="row py-1">
                                <div class="col-3">
                                    <strong>{{ ucfirst(__('my-crm::lang.to')) }}</strong>
                                </div>
                                <div class="col">
                            ...
                            <div class="row py-1">
                                <div class="col-3">
                                    <strong>{{ ucfirst(__('my-crm::lang.invoice_number')) }}</strong>
                                </div>
                                <div class="col">
                                    {{ $invoice->invoice_id }}
                                </div>
                            ...
                            <div class="row py-1">
                                <div class="col-3">
                                    <strong>{{ ucfirst(__('my-crm::lang.issued')) }}</strong>
                                </div>
                                <div class="col">
                            ...
                            <div class="row py-1">
                                <div class="col-3">
                                    <strong>{{ ucfirst(__('my-crm::lang.due')) }}</strong>
                                </div>
                                <div class="col">
                                    ...
                                    @if(! $invoice->fully_paid_at && $invoice->due_date->diffinDays() > 0 && $invoice->due_date >= \Carbon\Carbon::now()->timezone($timezone))
                                        <small class="text-secondary"> ({{ ucfirst(__('my-crm::lang.due_in')) }} {{ $invoice->due_date->diffForHumans(false, true) }})</small>
                                    @elseif(! $invoice->fully_paid_at && $invoice->due_date->diffinDays() <= 0  && $invoice->due_date >= \Carbon\Carbon::now()->timezone($timezone))
                                        <small class="text-secondary"> ({{ ucfirst(__('my-crm::lang.due_tomorrow')) }}</small>
                                    @elseif(! $invoice->fully_paid_at && $invoice->due_date < \Carbon\Carbon::now()->timezone($timezone))
                                        <small class="text-danger"> ({{ $invoice->due_date->diffForHumans(false, true) }} {{ __('my-crm::lang.overdue') }})</small>
                                    @endif
                                </div>
                            </div>
                            @endif    
                    ...
                    @if($paymentInstructions)
                        <div class="row py-1">
                            <div class="col px-5 py-4">
                                <h5>{{ ucfirst(__('my-crm::lang.payment')) }}</h5>
