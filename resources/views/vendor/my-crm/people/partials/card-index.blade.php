@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.people')) }}
        @endslot
    
        @slot('actions')
            @include('my-crm::partials.filters', [
                'action' => route('laravel-crm.people.filter'),
                'model' => '\Kv\MyCrm\Models\Person'
            ])
            @can('create crm people')
            <span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.people.create')) }}"><span class="fa fa-plus"></span>  {{ ucfirst(__('my-crm::lang.add_person')) }}</a></span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        ...
        <table class="table mb-0 card-table table-hover">
            <thead>
            <tr>
                <th scope="col">@sortablelink('first_name', ucwords(__('my-crm::lang.name')))</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.labels')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.email')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.phone')) }}</th>
            ...
            @foreach($people as $person)
                <tr class="has-link" data-url="{{ url(route('laravel-crm.people.show',$person)) }}">
                    ...
                    <td></td>
                    <td>{{ $person->ownerUser->name ?? ucfirst(__('my-crm::lang.unallocated')) }}</td>
