@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.orders')) }}
        @endslot

        @slot('actions')
            @include('my-crm::partials.filters', [
                'action' => route('laravel-crm.orders.filter'),
                'model' => '\Kv\MyCrm\Models\Order'
            ])
            @can('create crm orders')
            <span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.orders.create')) }}"><span class="fa fa-plus"></span>  {{ ucfirst(__('my-crm::lang.add_order')) }}</a></span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        ...
        <table class="table mb-0 card-table table-hover">
            ...
            <tr>
                ...
                @hasquotesenabled
                <th scope="col">{{ ucwords(__('my-crm::lang.quote')) }}</th>
                @endhasquotesenabled
            ...
            @foreach($orders as $order)
               ...
               <tr class="has-link" data-url="{{ url(route('laravel-crm.orders.show', $order)) }}">
                   ...
                   <td>@include('my-crm::partials.labels',[
                    ...
                    <td>{{ $order->ownerUser->name ?? ucfirst(__('my-crm::lang.unallocated')) }}</td>
                        ...
                        @can('edit crm orders')
                            ...
                            @else
                                @if(! $order->deliveryComplete())
                                    ...
                                    <a href="{{ route('laravel-crm.purchase-orders.create',['model' => 'order', 'id' => $order->id]) }}" class="btn btn-success btn-sm">{{ ucwords(__('my-crm::lang.purchase')) }}</a>
                                    @endhaspurchaseordersenabled
                                @endif
                                @if(! $order->invoiceComplete())
                                    @hasinvoicesenabled
                                    <a href="{{ route('laravel-crm.invoices.create',['model' => 'order', 'id' => $order->id]) }}" class="btn btn-success btn-sm">{{ ucwords(__('my-crm::lang.invoice')) }}</a>
                                    @endhasinvoicesenabled
                                @endif        
                                @if(! $order->deliveryComplete())
                                    @hasdeliveriesenabled
                                        <a href="{{ route('laravel-crm.deliveries.create',['model' => 'order', 'id' => $order->id]) }}" class="btn btn-success btn-sm">{{ ucwords(__('my-crm::lang.delivery')) }}</a>
