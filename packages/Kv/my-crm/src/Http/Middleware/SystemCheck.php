        ...
        if(config('laravel-crm.update_notifications')) {
            ...
            if(! auth()->guest() && Schema::hasTable(config('laravel-crm.db_table_prefix').'settings') && auth()->user()->hasPermissionTo('view crm updates')) {
                if(\Kv\MyCrm\Models\Setting::where('name', 'version')->first()->value < \Kv\MyCrm\Models\Setting::where('name', 'version_latest')->first()->value) {
                    flash('There is a new version of Laravel CRM software available. <a href="https://github.com/venturedrake/laravel-crm" target="_blank">View version '.\Kv\MyCrm\Models\Setting::where('name', 'version_latest')->first()->value.' details</a> or <a href="https://github.com/venturedrake/laravel-crm" target="_blank">update now</a>.')->warning()->important();
                }

                // Check if DB database required
                $dbUpdateRequired = false;

                if($setting = \Kv\MyCrm\Models\Setting::where('name', 'db_update_0180')->first()) {
                    if($setting->value == 0) {
                        $dbUpdateRequired = true;
                    }
                }

                if($setting = \Kv\MyCrm\Models\Setting::where('name', 'db_update_0181')->first()) {
                    if($setting->value == 0) {
                        $dbUpdateRequired = true;
                    }
                }

                if($setting = \Kv\MyCrm\Models\Setting::where('name', 'db_update_0191')->first()) {
                    if($setting->value == 0) {
                        $dbUpdateRequired = true;
                    }
                }

                if($setting = \Kv\MyCrm\Models\Setting::where('name', 'db_update_0193')->first()) {
                    if($setting->value == 0) {
                        $dbUpdateRequired = true;
                    }
                }

                if($setting = \Kv\MyCrm\Models\Setting::where('name', 'db_update_0194')->first()) {
                    if($setting->value == 0) {
                        $dbUpdateRequired = true;
                    }
                }

                if($setting = \Kv\MyCrm\Models\Setting::where('name', 'db_update_0199')->first()) {
                    if($setting->value == 0) {
                        $dbUpdateRequired = true;
                    }
                }

                if($setting = \Kv\MyCrm\Models\Setting::where('name', 'db_update_1200')->first()) {
                    if($setting->value == 0) {
                        $dbUpdateRequired = true;
                    }
                }
