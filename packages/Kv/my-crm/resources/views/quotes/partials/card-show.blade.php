    ...
    @component('my-crm::components.card-header')
        ...
        @slot('actions')
            <span class="float-right">
                @php
                    (!\Kv\MyCrm\Http\Helpers\CheckAmount\subTotal($quote) || ! \Kv\MyCrm\Http\Helpers\CheckAmount\total($quote)) ? $quoteError = true : $quoteError = false;
                @endphp
                ...
                @can('edit crm quotes')
                    ...
                    @else
                        @if(!$quote->accepted_at && !$quote->rejected_at)
                            <a href="{{ route('laravel-crm.quotes.accept',$quote) }}" class="btn btn-success btn-sm">{{ ucfirst(__('my-crm::lang.mark_as_accepted')) }}</a>
                            <a href="{{ route('laravel-crm.quotes.reject',$quote) }}" class="btn btn-danger btn-sm">{{ ucfirst(__('my-crm::lang.mark_as_rejected')) }}</a>
                        @elseif($quote->accepted_at && $quote->orders()->count() > 0 && ! $quote->orderComplete())
                            @hasordersenabled
                                <a href="{{ route('laravel-crm.orders.create',['model' => 'quote', 'id' => $quote->id]) }}" class="btn btn-success btn-sm">{{ ucfirst(__('my-crm::lang.create_order')) }}</a>
                            @endhasordersenabled
                        @elseif($quote->accepted_at && $quote->orders()->count() < 1)
                            <a href="{{ route('laravel-crm.quotes.unaccept',$quote) }}" class="btn btn-outline-secondary btn-sm">{{ ucfirst(__('my-crm::lang.unaccept')) }}</a>
                            @hasordersenabled
                            <a href="{{ route('laravel-crm.orders.create',['model' => 'quote', 'id' => $quote->id]) }}" class="btn btn-success btn-sm">{{ ucfirst(__('my-crm::lang.create_order')) }}</a>
                            @endhasordersenabled
                        @elseif($quote->rejected_at)
                            <a href="{{ route('laravel-crm.quotes.unreject',$quote) }}" class="btn btn-outline-secondary btn-sm">{{ ucfirst(__('my-crm::lang.unreject')) }}</a>
                        @endif
                    @endif    
                @endcan
                @can('view crm quotes')
                    @if(! $quoteError)
                    <a class="btn btn-outline-secondary btn-sm" href="{{ route('laravel-crm.quotes.download', $quote) }}">{{ ucfirst(__('my-crm::lang.download')) }}</a>
                    @endif
                @endcan    
                ...
                <form action="{{ route('laravel-crm.quotes.destroy',$quote) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                    {{ method_field('DELETE') }}
                    {{ csrf_field() }}
                    <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.quote') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                </form>
                @endcan
            </span>
        @endslot
    ...
    @component('my-crm::components.card-body')
        ...
        <div class="row card-show card-fa-w30">
            <div class="col-sm-6 border-right">
                ...
                <hr />
                <dl class="row">
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.number')) }}</dt>
                    ...
                    <dd class="col-sm-9">@include('my-crm::partials.labels',[
                            'labels' => $quote->labels
                    ])</dd>
                    <dt class="col-sm-3 text-right">Owner</dt>
                    <dd class="col-sm-9">
                        @if($quote->ownerUser)<a href="{{ route('laravel-crm.users.show', $quote->ownerUser) }}">{{ $quote->ownerUser->name ?? null }}</a> @else  {{ ucfirst(__('my-crm::lang.unallocated')) }} @endif
                    </dd>
                </dl>
                ...
                <h6 class="text-uppercase mt-4 section-h6-title-table"><span>{{ ucfirst(__('my-crm::lang.quote_items')) }} ({{ $quote->quoteProducts->count() }})</span></h6>
                <table class="table table-hover">
                    ...
                    @foreach($quote->quoteProducts()->whereNotNull('product_id')->get() as $quoteProduct)
                        <tr>
                            ...
                            <td>
                                @if(! \Kv\MyCrm\Http\Helpers\CheckAmount\lineAmount($quoteProduct))
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td>
                                @if(! \Kv\MyCrm\Http\Helpers\CheckAmount\subTotal($quote))
                        ...
                        <tr>
                            ...
                            <td></td>
                            <td></td>
                            <td><strong>{{ ucfirst(__('my-crm::lang.tax')) }}</strong></td>
                            <td>
                                @if(! \Kv\MyCrm\Http\Helpers\CheckAmount\tax($quote))
                        ...
                        <tr>
                            ...
                            <td>
                                @if(! \Kv\MyCrm\Http\Helpers\CheckAmount\total($quote))
