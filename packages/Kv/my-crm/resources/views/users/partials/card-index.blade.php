@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.users')) }}
        @endslot

        @slot('actions')
            @can('create crm users')
            <span class="float-right">
                <a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.users.create')) }}"><span class="fa fa-plus"></span> {{ ucfirst(__('my-crm::lang.add_user')) }}</a>
                {{--<a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.users.invite')) }}"><span class="fa fa-paper-plane"></span> {{ ucfirst(__('my-crm::lang.invite_user')) }}</a>--}}
            </span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        ...
        <table class="table mb-0 card-table table-hover">
            <thead>
            <tr>
                <th scope="col">{{ ucwords(__('my-crm::lang.name')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.email')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.email_verified')) }}</th>
                <th scope="col">{{ __('my-crm::lang.CRM_Access') }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.role')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.created')) }}</th>
                {{-- <th scope="col">{{ ucfirst(__('my-crm::lang.updated')) }}</th>--}}
                <th scope="col">{{ ucwords(__('my-crm::lang.last_online')) }}</th>
