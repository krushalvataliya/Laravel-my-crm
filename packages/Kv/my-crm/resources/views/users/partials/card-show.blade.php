    ...
    @component('my-crm::components.card-header')
        ...
        @slot('actions')
            <span class="float-right">
                <a type="button" class="btn btn-outline-secondary btn-sm" href="{{ url(route('laravel-crm.users.index')) }}"><span class="fa fa-angle-double-left"></span> {{ ucfirst(__('my-crm::lang.back_to_users')) }}</a> | 
                @can('edit crm users')
                ...
                <form action="{{ route('laravel-crm.users.destroy',$user) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                    {{ method_field('DELETE') }}
                     {{ csrf_field() }}
                    <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.user') }}" {{ (auth()->user()->id == $user->id) ? 'disabled' : null }}><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                </form>
                @endcan    
            </span>
        @endslot
        ...
        <div class="row">
            <div class="col-sm-6 border-right">
                ...
                <dl class="row">
                    ...
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.email')) }}</dt>
                    <dd class="col-sm-9">{{ $user->email }}</dd>
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.email_verified')) }}</dt>
                    <dd class="col-sm-9">{{ ($user->email_verified_at) ? $user->email_verified_at->format($dateFormat.' '.$timeFormat) : null }}</dd>
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.CRM_Access')) }}</dt>
                    <dd class="col-sm-9">{{ ($user->crm_access) ? 'Yes' : 'No' }}</dd>
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.CRM_Role')) }}</dt>
                    <dd class="col-sm-9">{{ $user->roles()->first()->name ?? null }}</dd>
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.created')) }}</dt>
                    <dd class="col-sm-9">{{ $user->created_at->format($dateFormat) }}</dd>
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.last_online')) }}</dt>
            ...
            <div class="col-sm-6">
                @hasteamsenabled
                @can('view crm teams')
                    <h6 class="text-uppercase section-h6-title"><span>{{ ucfirst(__('my-crm::lang.teams')) }} ({{ $user->crmTeams->count() }})</span></h6>
                    <hr />
