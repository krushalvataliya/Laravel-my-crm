...
@section('content')
    ...
    <table class="table table-sm table-items">
        <tbody>
            <tr>
                <td width="50%"> 
                    <h1>{{ strtoupper(__('my-crm::lang.delivery')) }}</h1>
                    @if($order->reference || $delivery->delivery_expected)
                    <p>
                    @endif    
                        @if($order->reference)
                            <strong>{{ ucfirst(__('my-crm::lang.reference')) }}</strong> {{ $order->reference }}
                        @endif
                        @if($delivery->delivery_expected)
                            @if($order->reference)
                                <br />
                            @endif    
                            <strong>{{ ucwords(__('my-crm::lang.delivery_date')) }}</strong> {{ $delivery->delivery_expected }}
                        @endif
                    @if($order->reference || $delivery->delivery_expected)
                    </p>
                   @endif
            ...
            <tr>
                <td>
                    ...
                    @if($address && $address->contact)
                        <strong>{{ ucwords(__('my-crm::lang.contact')) }}: {{ $address->contact }}</strong><br >
                    @endif
                    @if($address && $address->phone)
                        <strong>{{ ucwords(__('my-crm::lang.phone')) }}: {{ $address->phone }}</strong><br >
                    @endif
                    @if($address)
                        {{ $address->line1 }}<br />
                        @if($address->line2)
    ...
    <table class="table table-bordered table-sm table-delivery">
        <tbody>
        <tr>
            <th width="150">{{ ucfirst(__('my-crm::lang.received_by')) }} </th>
            <td></td>
        </tr>
        <tr>
            <th>{{ ucfirst(__('my-crm::lang.received_date')) }} </th>
            <td></td>
        </tr>
        <tr>
            <th>{{ ucfirst(__('my-crm::lang.signature')) }} </th>
            <td></td>
        </tr>
        </tbody>
    </table>
