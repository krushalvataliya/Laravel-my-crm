            ...
            <div class="col col-md-10">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title float-left m-0"> {{ ucfirst(__('my-crm::lang.pipeline_stages')) }} </h3> @can('create crm pipelines')<span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.pipeline-stages.create')) }}"><span class="fa fa-plus"></span>  {{ ucfirst(__('my-crm::lang.add_pipeline_stage')) }}</a></span>@endcan
                    </div>
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0 card-table table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col">{{ ucfirst(__('my-crm::lang.pipeline')) }}</th>
                                        <th scope="col"></th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                            ...
                                            <td class="disable-link text-right">
                                                ...
                                                @can('delete crm pipelines')
                                                    <form action="{{ route('laravel-crm.pipeline-stages.destroy',$pipelineStage) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                                                        {{ method_field('DELETE') }}
                                                        {{ csrf_field() }}
                                                        <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.pipeline_stage') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                                                    </form>
                                                @endcan
                                            </td>
                                        </tr>
