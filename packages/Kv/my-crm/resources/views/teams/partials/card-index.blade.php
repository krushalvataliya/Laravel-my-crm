@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.teams')) }}
        @endslot

        @slot('actions')
            @can('create crm teams')
            <span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.teams.create')) }}"><span class="fa fa-plus"></span> {{ ucfirst(__('my-crm::lang.add_team')) }}</a></span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        ...
        <table class="table mb-0 card-table table-hover">
            ...
            <tr>
                ...
                <th scope="col">{{ ucwords(__('my-crm::lang.created_by')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.created')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.updated')) }}</th>
                <th scope="col">{{ ucwords(__('my-crm::lang.users')) }}</th>
                <th scope="col"></th>
            </tr>
            </thead>
            <tbody>
