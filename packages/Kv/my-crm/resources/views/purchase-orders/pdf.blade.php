...
@section('content')
    ...
    <table class="table table-sm table-items">
        <tbody>
            <tr>
                <td width="50%"> 
                    <h1>{{ strtoupper(__('my-crm::lang.purchase_order')) }}</h1>
                    <p>
                    <strong>{{ ucfirst(__('my-crm::lang.purchase_order_date')) }}</strong> {{ $purchaseOrder->issue_date->format($dateFormat) }}<br />
                    <strong>{{ ucfirst(__('my-crm::lang.purchase_order_number')) }}</strong> {{ $purchaseOrder->xeroPurchaseOrder->number ?? $purchaseOrder->purchase_order_id  }}
                    @if($purchaseOrder->delivery_date)
                    <br /><strong>{{ ucfirst(__('my-crm::lang.delivery_date')) }}</strong> {{ $purchaseOrder->delivery_date->format($dateFormat) }}
                    @endif
                    @if($purchaseOrder->reference || ($purchaseOrder->xeroPurchaseOrder && $purchaseOrder->xeroPurchaseOrder->reference))
                    <br /><strong>{{ ucfirst(__('my-crm::lang.reference')) }}</strong> {{ $purchaseOrder->xeroPurchaseOrder->reference ?? $purchaseOrder->reference }}<br />
                    @endif
                    </p>
                </td>
            ...
            <tr>
                <td>
                    <strong>{{ ucfirst(__('my-crm::lang.supplier')) }}</strong><br />
                ...
                <td>
                    <strong>{{ ucfirst(__('my-crm::lang.delivery_details')) }}</strong><br />
                    {{ $fromName }}<br />
                    @if($purchaseOrder->delivery_type == 'pickup')
                        {{ strtoupper(__('my-crm::lang.pickup')) }}
                    @else    
                        @if($purchaseOrder->address)
                            ...
                            <br /><strong>{{ ucfirst(__('my-crm::lang.delivery_contact')) }}</strong><br />
                            {{ $purchaseOrder->address->contact }}
                            <strong>{{ ucfirst(__('my-crm::lang.delivery_phone')) }}</strong><br />
    ...
    @if($purchaseOrder->delivery_instructions)
        <table class="table table-bordered table-sm table-items">
            ...
            <tr>
                <td>
                    <h4>{{ ucfirst(__('my-crm::lang.delivery_instructions')) }}</h4>
