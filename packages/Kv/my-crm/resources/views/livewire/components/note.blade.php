            ...
            <div class="media-body">
                @if($note->relatedNote)
                    <h5 class="mt-0 mb-1">{{ $note->relatedNote->created_at->diffForHumans() }}
                        - {{ $note->relatedNote->createdByUser->name }} @include('my-crm::livewire.components.partials.note.actions', ['note' => $note])</h5>
                    <p class="pb-0 mb-2">
                        @if($note->relatedNote->noteable instanceof \Kv\MyCrm\Models\Person)
                        ...
                        @elseif($note->relatedNote->noteable instanceof \Kv\MyCrm\Models\Organisation)
                    ...
                    @include('my-crm::livewire.components.partials.note.content', ['note' => $note->relatedNote])
                @else
                    <h5 class="mt-0 mb-1">{{ $note->created_at->diffForHumans() }}
                        - {{ $note->createdByUser->name }} @include('my-crm::livewire.components.partials.note.actions', ['note' => $note])</h5>
                    @if($showRelated)
                        <p class="pb-0 mb-2">
                        @if($note->noteable instanceof \Kv\MyCrm\Models\Person)
                        ...
                        @elseif($note->noteable instanceof \Kv\MyCrm\Models\Organisation)
                    ...
                    @endif
                    @include('my-crm::livewire.components.partials.note.content', ['note' => $note])
                @endif
            </div>
        </div>
    </div>
