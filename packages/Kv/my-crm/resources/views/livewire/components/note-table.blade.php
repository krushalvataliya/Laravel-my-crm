<tr>
    ...
    <td>{{ $note->content }}<br />
        @include('my-crm::notes.partials.note-model', ['note' => $note]) 
    </td>
    <td>
        @if($note->noted_at)
    ...
    <td class="disable-link text-right">
        @can('edit crm notes')
            {{--@if(! $note->completed_at)
                <a href="{{  route('laravel-crm.notes.complete',$note) }}" class="btn btn-success btn-sm">{{ ucfirst(__('my-crm::lang.complete')) }}</a>
            @endif--}}
        ...
        <form action="{{ route('laravel-crm.notes.destroy',$note) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
            {{ method_field('DELETE') }}
            {{ csrf_field() }}
            <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.note') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
        </form>
        @endcan
    </td>
</tr>
