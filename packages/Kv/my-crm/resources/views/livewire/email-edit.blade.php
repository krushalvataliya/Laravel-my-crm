<div class="email-addresses">
    <h6 class="text-uppercase mt-4 section-h6-title"><span>{{ ucfirst(__('my-crm::lang.emails')) }}</span> <span class="float-right"><button class="btn btn-outline-secondary btn-sm" wire:click.prevent="add({{$i}})"><span class="fa fa-plus" aria-hidden="true"></span></button></span></h6>
    <hr />
        ...
        <div class="form-row">
            <div class="col-sm-6">
                <div class="form-group @error('emails.'.$value.'.address') text-danger @enderror">
                    <label>{{ ucfirst(__('my-crm::lang.email')) }}</label>
            ...
            <div class="col-sm-4">
                <div class="form-group @error('emails.'.$value.'.type') text-danger @enderror">
                    ...
                    <select class="form-control custom-select @error('emails.'.$value.'.type') is-invalid @enderror" wire:model="type.{{ $value }}" name="emails[{{ $value }}][type]">
                        @foreach(\Kv\MyCrm\Http\Helpers\SelectOptions\emailTypes() as $optionKey => $optionName)
