<div class="phone-numbers">
    <h6 class="text-uppercase section-h6-title"><span>{{ ucfirst(__('my-crm::lang.phone_numbers')) }}</span> <span class="float-right"><button class="btn btn-outline-secondary btn-sm" wire:click.prevent="add({{$i}})"><span class="fa fa-plus" aria-hidden="true"></span></button></span></h6>
    <hr />
        ...
        <div class="form-row">
            <div class="col-sm-6">
                <div class="form-group @error('phones.'.$value.'.number') text-danger @enderror">
                    <label>{{ ucfirst(__('my-crm::lang.phone')) }}</label>
            ...
            <div class="col-sm-4">
                <div class="form-group @error('phones.'.$value.'.type') text-danger @enderror">
                    ...
                    <select class="form-control custom-select @error('phones.'.$value.'.type') is-invalid @enderror" wire:model="type.{{ $value }}" name="phones[{{ $value }}][type]">
                        @foreach(\Kv\MyCrm\Http\Helpers\SelectOptions\phoneTypes() as $optionKey => $optionName)
