@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ ucfirst(__('my-crm::lang.clients')) }}
        @endslot

        @slot('actions')
            @include('my-crm::partials.filters', [
                'action' => route('laravel-crm.clients.filter'),
                'model' => '\Kv\MyCrm\Models\Client'
            ])
            @can('create crm clients')
                <span class="float-right"><a type="button" class="btn btn-primary btn-sm" href="{{ url(route('laravel-crm.clients.create')) }}"><span class="fa fa-plus"></span>  {{ ucfirst(__('my-crm::lang.add_client')) }}</a></span>
            @endcan
        @endslot
    ...
    @component('my-crm::components.card-table')
        ...
        <table class="table mb-0 card-table table-hover">
            <thead>
            <tr>
                <th scope="col">@sortablelink('name', ucwords(__('my-crm::lang.name')))</th>
                ...
                <tr class="has-link" data-url="{{ url(route('laravel-crm.clients.show',$client)) }}">
                    ...
                    <td class="disable-link text-right">
                        ...
                        <form action="{{ route('laravel-crm.clients.destroy',$client) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
                            <button class="btn btn-danger btn-sm" type="submit" data-model="{{ __('my-crm::lang.client') }}"><span class="fa fa-trash-o" aria-hidden="true"></span></button>
                        </form>
                        @endcan
                    </td>
                </tr>
