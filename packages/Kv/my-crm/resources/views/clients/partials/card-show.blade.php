@component('my-crm::components.card')
    ...
    @component('my-crm::components.card-header')
        ...
        @slot('title')
            {{ $client->name }} <small>@include('my-crm::partials.labels',[
        ...
        <div class="row">
            <div class="col-sm-6 border-right">
                ...
                <dl class="row">
                    <dt class="col-sm-3 text-right">{{ ucfirst(__('my-crm::lang.name')) }}</dt>
                    <dd class="col-sm-9">
                        @if($client->ownerUser)<a href="{{ route('laravel-crm.users.show', $client->ownerUser) }}">{{ $client->ownerUser->name ?? null }}</a> @else  {{ ucfirst(__('my-crm::lang.unallocated')) }} @endif
                    </dd>
                </dl>
